...
def detection_view(model, preproc):
    ...
    with col2:
        st.metric("Operating mode", "Recall-oriented")
...
def validation_view(model):
    ...
    if threshold <= 0.40:
        st.warning(
            "⚠️ High-Sensitivity Mode – maximises detection coverage by keeping recall "
            "extremely high, but produces many false positives. Suitable "
            "when missing an attack is considered more harmful than raising extra alerts."
        )

    elif 0.40 < threshold <= 0.70:
        st.info(
            "🟡 Recall-Oriented Mode – slightly reduces alert volume while still detecting "
            "most attacks. Precision remains modest due to dataset "
            "imbalance. This is a reasonable operating region for research IDS prototypes "
            "where analyst review is acceptable."
        )

    else:
        st.error(
            "🔴 Conservative Mode – suppresses false positives by raising the threshold, "
            "but recall drops sharply. Only strong, high-confidence attacks "
            "are detected, and many subtle intrusions may be missed."
        )
...
def inject_css():
    st.markdown(
        ...
        h1 {
        margin-top: -3rem !important;   /* adjust value */
        }
        </style>
        """,
        unsafe_allow_html=True,
