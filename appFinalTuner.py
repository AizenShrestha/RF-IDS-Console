...
def detection_view(model, preproc):
    ...
    # Same mode description as validation page
    if threshold <= 0.40:
        st.warning(
    ...
    # Threshold card (now reflects current threshold)
    st.markdown(
        f"""
        <div class="rf-card">
            <span class="rf-pill">Current threshold {threshold:.2f}</span>
            <p style="margin-top:0.6rem;">
                The detection console applies the selected decision threshold of
                <strong>{threshold:.2f}</strong> to convert attack probabilities into
                binary decisions. Lower thresholds increase sensitivity (more attacks
                flagged, more alerts), while higher thresholds reduce alerts at the
                cost of potentially missing low-confidence intrusions.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    # Upload instructions card (unchanged)
    st.markdown(
        """
        <div class="rf-card">
    ...
    if st.button("🚀 Run IDS Prediction"):
        try:
            # 🔁 Use the slider-selected threshold here (changed from DETECTION_THRESHOLD)
            result_df, dropped_cols, mode = make_predictions(
                df, model, preproc, threshold=threshold
            )
        except Exception as e:
            st.error(e)
            return
        ...
        st.caption(
            f"Predictions generated using decision threshold **{threshold:.2f}**."
        )
